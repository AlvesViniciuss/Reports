...
function calculateProduction(data, dateFilter) {
    ...
    data.forEach(item => {
        const equipment = item['Crane CHE Name'];
        const moveType = item['Move Kind'];
        const operator = item['OperadorPT'];
        const timestamp = item['Time Completed'];

        // Skip processing if equipment or operator is blank
        if (!equipment || !operator) {
            return;
        }

        // Ignore payload moves
        if (moveType.includes('Payload')) {
            return;
        }
        ...
        switch (moveType) {
            case 'Load':
                productionCount.total[equipment].UNIT_LOAD++;
                productionCount.operator[operator].UNIT_LOAD++;
                productionCount.shift[shift].UNIT_LOAD++;
                break;
            case 'Discharge':
                productionCount.total[equipment].UNIT_DISCH++;
                productionCount.operator[operator].UNIT_DISCH++;
                productionCount.shift[shift].UNIT_DISCH++;
                break;
            default:
                break;
        }
...
function displayTable(data, tableId, showTotalColumn, sortBy) {
    ...
    const columnsOrder = ['UNIT_LOAD', 'UNIT_DISCH', 'TOTAL'];
    ...
    sortedKeys.forEach(key => {
        // Skip rows where operator is blank
        if (!key.trim()) {
            return;
        }
...
function displayUtilizationChart(totalData) {
    ...
    for (const equipment in totalData) {
        totalMovements += totalData[equipment].UNIT_LOAD +
                          totalData[equipment].UNIT_DISCH;
    }
    ...
    equipmentLabels.forEach(equipment => {
        const total = totalData[equipment].UNIT_LOAD +
                      totalData[equipment].UNIT_DISCH;

        const percentage = ((total / totalMovements) * 100).toFixed(2);
    ...
    utilizationData.sort((a, b) => b.percentage - a.percentage);

    const labels = utilizationData.map(item => item.equipment);
    const data = utilizationData.map(item => item.percentage);
    ...
    const chartConfig = {
        ...
        data: {
            labels: labels,
            datasets: [{
                label: 'Percentual de Movimentações',
                data: data,
        ...
        options: {
            ...
            title: {
                display: true,
                text: 'Percentual de Movimentações por Equipamento'
            }
        }
    };
...
function getTotalCount(data) {
    let totalCount = 0;
    totalCount += data.UNIT_LOAD || 0;
    totalCount += data.UNIT_DISCH || 0;
    return totalCount;
}
